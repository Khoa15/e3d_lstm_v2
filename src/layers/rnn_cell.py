...
class EideticLSTMCell(object):
  ...
  def _norm(self, inp, scope, dtype=tf.float32):
    ...
    gamma_init = tf.compat.v1.constant_initializer(self._norm_gain)
    beta_init = tf.compat.v1.constant_initializer(self._norm_shift)
    with tf.compat.v1.variable_scope(scope):
      # Initialize beta and gamma for use by layer_norm.
      tf.compat.v1.get_variable("gamma", shape=shape, initializer=gamma_init, dtype=dtype)
      tf.compat.v1.get_variable("beta", shape=shape, initializer=beta_init, dtype=dtype)
    normalized = slim.layer_norm(inp, reuse=True, scope=scope)
    return normalized
  ...
  def __call__(self, inputs, hidden, cell, global_memory, eidetic_cell):
    with tf.compat.v1.variable_scope(self._layer_name):
