...
class Model(object):
  ...
  def __init__(self, configs):
    ...
    self.real_input_flag = tf.compat.v1.placeholder(tf.float32, [
    ...
    self.itr = tf.compat.v1.placeholder(tf.float32, shape=[])
    self.params = dict()
    ...
    for i in range(self.configs.n_gpu):
      with tf.device('/gpu:%d' % i):
        with tf.compat.v1.variable_scope(
            tf.compat.v1.get_variable_scope(), reuse=True if i > 0 else None):
          # define a model
